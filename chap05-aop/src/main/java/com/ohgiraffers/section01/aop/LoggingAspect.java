...
public class LoggingAspect {
    ...
    /*
    @Aspect
     - pointcut과 advice를 하나의 클래스 단위로 정의하기 위한 어노테이션이다.

     조인 포인트 - aop에서 특정한 실행 지점을 의미한다.
     종류
     ...
     수 많은 조인 포인트 중에 특정 메소드만 횡단 공통기능을 수행하기 위해 사용한다.
     advice = 횡단 관심에 해당하는 공통 기능의 코드, 독립된 메소드로 작동
    * */


    /*
    * pointcut = 관심 조인 포인트를 결정하여 어드바이스가 실행되는 기시를 제어한다.
    * excution = 메소드 실행 조인 포인르를 매칭한다.
    *
    * execution (* 리턴타입. 이름(파라미터))
    * *Service.*(..) = 매개변수가 0개 이상인 모든 메소드
    * *Service.*(*) = 매개변구가 1개인 모든 메소드
    * *Service.*(*,*) = 매개변수가 2개인 모든 메소드
    * */
    @Pointcut("execution(* com.ohgiraffers.section01.aop.*Service.*(..))")
    public void logPointCut(){
        // 이건 어떤 상황에서 동작을 시킬지 지정만 하고 있다.
        System.out.println("이거 실행 안 되는 거야.. 그러니 볼 수 없겠지..");
    }
    ...
    * JoinPoint의 하위 인터페이스로 원본 조인포인트의 진행 시점을 제어할 수 있다.*/
    ...
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 원본 메소드 실행
        Object result = joinPoint.proceed(); // 프로시드를 만나야.. 원본이 실행된다. 프로시드가 없으면 상기 코드들은 의미를 갖지 못한다.
        if(result != null && result instanceof Map){
